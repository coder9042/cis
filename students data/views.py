...
def resetpass(request):
    if request.method == 'POST':
        if request.POST.get('e_id',''):
            ...
            p = Professor.objects.get(Email=pid)
            r = resetTable.objects.get(prof=p)
            code_actual = r.hex_code
            if code == code_actual:
                return render(request,'reset.html',{'type':3, 'prof':p.Email})
            else:
...
def login(request,isstud=None):
    ...
    if('user' in request.session.keys()):
        ...
        return render(request,'welcome.html',{'user':User, 'message':'', 'isstud':isstud})
    ...
    if('user' not in request.session.keys()):
        ...
        if request.method=='POST':
            ...
            if len(errors)==0:
                ...
                User=Professor.objects.filter(Username=username)[0]
                return render(request,'settings.html',{'user':User,'errors':errors})
        else:
            User=Professor.objects.filter(Username=username)[0]
            return render(request, 'settings.html',{'user':User,'errors':errors})
...
def studlogin(request):
    ...
    if('user' in request.session.keys()):
        ...
        return render(request,'studwelcome.html',{'user':User, 'message':'', 'isstud':isstud})
    message='Wrong Username or Password'
    if request.method=='POST':
        name=request.POST['username']
        password=request.POST['pass']
...
##                    )
                ...
                return render(request, 'Appointments.html',{'user':username,'page':2,'appointments':studApp})
            else:
                return render(request, 'Appointments.html',{'user':username,'page':1,'errors':errors,'data':data,'profs':Profs,'months':Months,'days':Days,'hours':Hours,'minutes':Minutes})
        #stud = Students.objects.filter(Username=username)[0]
        #studApp=stud.appointment_set.all()
        #if studApp == null:
        ...
        stud = Students.objects.filter(Username=username)[0]
        studApp=stud.appointment_set.all()
        return render(request, 'Appointments.html', {'user':username, 'page':offset, 'appointments':studApp, 'profs':Profs,'months':Months,'days':Days,'hours':Hours,'minutes':Minutes})
...
def changePassword(request):
    ...
    if request.method == 'POST' :
        ...
        if len(errors)==0:
            ...
            User.save()
            return render(request, 'studwelcome.html', {'user':User, 'errors':errors})
        return render(request, 'options.html', {'user':User, 'errors':errors})
    else:
        return render(request, 'options.html', {'user':User})
